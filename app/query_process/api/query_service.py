...
async def query(background_tasks: BackgroundTasks, request: QueryRequest):
    ...
    # 清理上一轮残留的任务状态（done_list/node_times/answer 等以 session_id 为 key 的字典）
    # 不清理的后果：同一会话多轮问答共用 key，Python dict 覆盖键值不会改变键的插入顺序，
    # 曾经历过「反问轮」（仅 confirm+answer_output 两个节点）的会话，生成答案会永远排在旧键位，
    # 导致前端展示顺序错乱（如生成答案出现在第二行）。
    ...
    if is_stream:
        ...
        return {
            ...
            "answer":answer,
            "done_list":[],
            "node_times":get_task_node_times(session_id)
        }
