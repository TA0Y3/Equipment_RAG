...
'''
_tasks_status = {
    "sess-abc123": "processing",   # 正在处理中
    "sess-xyz789": "completed",    # 已完成
    "sess-def456": "failed",       # 失败了
}
'''
...
_tasks_result = {
    "sess-abc123": {
        "answer": "HAK 180 烫金机的操作面板位于机器正前方...",  # LLM生成的最终答案
    },
    "sess-xyz789": {
        "answer": "抱歉，未找到相关产品...",
    },
...
# key: task_id
# value: {节点名(原始ID): 开始时间戳(perf_counter)}
# 节点计时起点（add_running_task 时记录，add_done_task 时取走）
_tasks_node_start: Dict[str, Dict[str, float]] = {}

# key: task_id
# value: {节点名(原始ID): 可读耗时字符串（如 "1.23s" / "123.45ms"）}
# 节点计时结果（仅记录已标记完成的节点）
_tasks_node_times: Dict[str, Dict[str, str]] = {}
...
def _ensure_task(task_id: str) -> None:
    ...
    if task_id not in _tasks_node_start:
        _tasks_node_start[task_id] = {}
    if task_id not in _tasks_node_times:
        _tasks_node_times[task_id] = {}
...
def add_running_task(task_id: str, node_name: str, is_stream: bool = False) -> None:
    ...
    # 记录节点开始时间（计时起点，从节点真正开始执行算起）
    _tasks_node_start[task_id][node_name] = time.perf_counter()
    #只要 is_stream=True，就自动触发一次推送。
    if is_stream:
...
def add_done_task(task_id: str, node_name: str, is_stream: bool = False) -> None:
    ...
    # 节点耗时统计：取走开始时间（pop 保证重复标记 done 不会覆盖首次耗时）
    # 未记录开始时间（如异常中断/直接标记完成）时跳过，不报错
    start_time = _tasks_node_start.get(task_id, {}).pop(node_name, None)
    if start_time is not None:
        _tasks_node_times[task_id][node_name] = _format_duration(time.perf_counter() - start_time)
...
def _format_duration(seconds: float) -> str:
    """
    将秒数格式化为可读耗时字符串：>=1 秒显示 "1.23s"，不足 1 秒显示 "123.45ms"。
    """
    if seconds >= 1:
...
def get_task_node_times(task_id: str) -> Dict[str, str]:
    """
    获取各节点耗时（中文节点名 -> 耗时字符串，与 done_list 展示名一致）。

    仅返回已标记完成的节点耗时；异常中断、未走到 add_done_task 的节点不返回，不抛错。
    """
    times = _tasks_node_times.get(task_id, {})
    return {_to_cn(node): duration for node, duration in times.items()}
...
def task_push_queue(task_id: str):
    push_to_session(task_id, "progress", {
        ...
        "node_times": get_task_node_times(task_id),
    })
...
def clear_task(task_id: str):
    ...
    _tasks_node_start.pop(task_id, None)
    _tasks_node_times.pop(task_id, None)