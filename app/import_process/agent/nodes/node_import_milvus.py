...
def step_3_clean_old_data(client, chunks_json_data: List[Dict[str, Any]]):
    """
    步骤3：幂等性处理 - 基于item_name+file_title复合键清理旧数据（细粒度去重）
    核心设计：
        插入新数据前删除同(item_name + file_title)组合的所有旧切片，确保多次执行仅保留最新数据
        支持多商品+多文件批量清理，自动去重避免重复操作
        细粒度优势：同一商品名的多个不同文件可以共存，互不影响
    参数：
        client - MilvusClient实例
        chunks_json_data: List[Dict[str, Any]] - 待入库的切片列表
    """
    # 提取唯一的 (item_name, file_title) 组合，实现细粒度去重
    # - 使用集合(set)自动去重，避免重复清理相同的商品-文件组合
    # - 海象操作符 ( := ) 同时提取和判断，简洁高效
    unique_keys = sorted({
        ((str(x.get("item_name", "")).strip()),
         (str(x.get("file_title", "")).strip()))
        for x in chunks_json_data or []
        if str(x.get("item_name", "")).strip() and str(x.get("file_title", "")).strip()
    })

    # 无有效组合则跳过清理
    if not unique_keys:
        logger.warning("Milvus幂等性清理跳过：切片中无有效的(item_name, file_title)组合")
        return
    # 多组合提示日志
    if len(unique_keys) > 1:
        logger.info(f"Milvus幂等性清理：本次检测到{len(unique_keys)}个不同的(商品名, 文件标题)组合，将逐个清理")
        for idx, (item, file) in enumerate(unique_keys[:5], 1):  # 只显示前5个，避免日志过长
            logger.info(f"  组合{idx}: [{item}] - [{file}]")
        if len(unique_keys) > 5:
            logger.info(f"  ... 还有 {len(unique_keys) - 5} 个组合")

    # 遍历每个唯一组合，逐个清理旧数据
    for item_name, file_title in unique_keys:
        _clear_chunks_by_composite_key(client, CHUNKS_COLLECTION_NAME, item_name, file_title)


def _clear_chunks_by_composite_key(client, collection_name: str, item_name: str, file_title: str):
    """
    内部核心函数：根据(item_name + file_title)复合键删除Milvus中的旧切片数据（细粒度清理）
    核心优势：
        精准定位到特定商品的特定文件，不会误删其他文件的相同商品数据
        例如：删除"iPhone 15 Pro_用户手册.pdf"的数据时，不会影响"iPhone 15 Pro_技术规格.pdf"的数据
    参数：
        client - MilvusClient实例
        collection_name: str - 集合名称
        item_name: str - 要清理的商品名称
        file_title: str - 要清理的源文件标题
    异常：
        清理失败抛出ValueError，终止整个入库流程（保证幂等性）
    """
    # 预处理：去除空格，空值直接返回
    i_name = (item_name or "").strip()
    f_title = (file_title or "").strip()

    if not i_name:
        logger.warning("Milvus复合键清理跳过：item_name为空")
        return
    if not f_title:
        logger.warning("Milvus复合键清理跳过：file_title为空")
        return
    if not collection_name:
        logger.warning("Milvus复合键清理跳过：集合名称未配置")
        return

    try:
        # 集合不存在则无需清理
        if not client.has_collection(collection_name=collection_name):
            logger.info(f"Milvus复合键清理跳过：集合{collection_name}不存在")
            return

        # 1. 商品名称和文件标题安全转义，防止特殊字符导致filter表达式解析失败
        safe_item_name = escape_milvus_string(i_name)
        safe_file_title = escape_milvus_string(f_title)

        # 构建复合过滤条件：同时匹配item_name和file_title（AND逻辑）
        filter_expr = f'item_name == "{safe_item_name}" AND file_title == "{safe_file_title}"'

        logger.info(
            f"Milvus幂等性清理（复合键）：开始删除集合[{collection_name}]中 item_name=[{i_name}] AND file_title=[{f_title}] 的旧数据")

        # 2. 执行删除操作（只删除完全匹配的记录）
        delete_result = client.delete(collection_name=collection_name, filter=filter_expr)
        deleted_count = delete_result.get('delete_count', 0) if isinstance(delete_result, dict) else 'unknown'
        logger.info(f"Milvus幂等性清理（复合键）：已标记删除 {deleted_count} 条旧记录")

        # 3. 强制flush，确保删除操作立即生效（避免Milvus异步延迟导致查询到脏数据）
        if hasattr(client, "flush"):
            try:
                client.flush(collection_name=collection_name)
                logger.debug(f"Milvus幂等性清理（复合键）：flush成功，删除操作已持久化")
            except Exception as e:
                logger.warning(f"Milvus幂等性清理（复合键）：flush操作失败，不影响主流程 | 错误：{str(e)}")

        logger.info(f"Milvus幂等性清理（复合键）完成：成功清理 [{i_name}] - [{f_title}] 的历史数据")
    except Exception as e:
        logger.error(f"Milvus幂等性清理（复合键）失败：item_name=[{i_name}], file_title=[{f_title}] | 错误：{str(e)}",
                     exc_info=True)
        raise ValueError(f"幂等清理失败（复合键：item_name={i_name}, file_title={f_title}）: {e}")
